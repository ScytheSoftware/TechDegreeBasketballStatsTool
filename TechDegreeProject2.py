...
def display_panthers(team_roster):
	...
	for person in team_roster:
		...
		for info in person:
			...
			#Guardians
			elif info == 'guardians' and (len(person[info]) >2) and 'and' not in person[info]:
				guardian_list.append(person[info])

			elif info == 'guardians' and 'and' in person[info]: #This is to drop the and in guardians
				person[info] = (person[info]).split('and')

				if info == 'guardians' and len(person[info]) > 1:#this checks to see if it's a mini list because of the split
					for names in person[info]:
						guardian_list.append(names)

			elif info == 'guardians' and len(person[info]) > 1:#This is here a second time because the user might go through the program again
					for names in person[info]:
						guardian_list.append(names)
...
if __name__ == '__main__':
	...
	for items in range(len(constants.PLAYERS)):

		data_placeholder = constants.PLAYERS[items]
		exp_placement = False
